...
class OjtAttendance(models.Model):
    ...
    # Relations: batch, event (optional), and participant
    ...
    # Presence & method: status of attendance and capture method
    ...
    # Duration (minutes): computed from check_in/out
    ...
    # Attendance %: 100 for present/late, 0 for absent; averaged in reports
    attendance_percent = fields.Float(
        string="Attendance %",
        compute="_compute_attendance_percent",
        store=True,
        group_operator="avg",
        help="100 for Present/Late, 0 for Absent. Aggregated average gives attendance percentage.",
    )
    ...
    # Onchange: infer presence from event start vs check-in
    @api.onchange("check_in", "event_link_id")
    def _onchange_presence(self):
        for rec in self:
            if rec.check_in and rec.event_link_id and getattr(rec.event_link_id, "date_start", False):
                rec.presence = "late" if rec.check_in > rec.event_link_id.date_start else "present"

    # Constraint: check_out must not be earlier than check_in
    ...
    # Constraint: participant and event must belong to the selected batch
